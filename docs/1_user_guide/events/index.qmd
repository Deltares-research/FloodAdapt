...
::: {.callout-note}
## Why don't I see hurricanes as an event type?
FloodAdapt uses two models: an overland model (required) and an offshore model (optional). The overland model simulates flooding from rainfall, river discharge, wind, and water levels—supporting synthetic and historical events. The offshore model adds the ability to simulate hurricanes by translating hurricane wind and pressure into nearshore water levels. If your system doesn’t include an offshore model, the Hurricane event type won’t appear.
:::
...
(1) An **overland model** that calculates the inundation over land due to nearshore water levels, local wind, rainfall, and river discharge. The overland model can be forced with historical (gauged) or synthetic water levels. Altneratively, an offshore model can calculate the nearshore water levels for specified time periods.
...
When a user selects a [Hurricane](historic_events/historic_events_hurricane.qmd), FloodAdapt will pull in wind and pressure information about the track, and use this information to generate wind and pressure fields using the [Holland 2010 method](https://journals.ametsoc.org/view/journals/mwre/138/12/2010mwr3317.1.xml). It also uses the track information to estimate the rainfall field using the [IPET method](https://www.sciencedirect.com/science/article/abs/pii/S0022169419309904). The wind and pressure fields are used by the offshore model to calculate nearshore water levels. These are then used, together with the same wind, pressure and rainfall fields, to run the overland flood model.
...
When a user selects a [Synthetic event](synthetic_events.qmd), only the overland model will be run, using the water level, rainfall, wind, and river discharge information specified by the user.
