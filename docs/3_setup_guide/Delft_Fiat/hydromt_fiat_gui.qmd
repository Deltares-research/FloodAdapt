...
*   [Update classification of buildings](#sec-classification)
...
#### Updating classification {#sec-classification}
The classification of a building defines the building type and can be divided into "primary"and "secondary" classification.
The primary classification gives a broad description of the building type such as "residential", "industrial", or "commercial". The secondary classification categorizes the buildings further into more refined groups based for instance on their number of stories, building material or other descriptors. Based on this classification the Delft-FIAT model-builder assigns the corresponding damage curve.

In some cases it can be desirable to update the classification of certain buildings with your own data and select a different damage curve.

To update the classification of using the Delft-FIAT model-builder:

1. Open the **Exposure** tab from the top panel and select the **Classification** tab in the bottom panel.
2. Click the drop-down menu in the "classification Source"-window and select "Upload data".
3. Click the "Load source" button and choose the point or polygon file with your classification data. This will load the data field names in your file into the "Assign classification" drop-down menu.
4. Select whether you want to update the primary or secondary object type in the first drop-down menu.
5. Select the field name that corresponds to your classification data  in the second drop-down menu and click "Add".
6. Standardize your data:
    a) Click "OK" on the info box and a window will open to assign a damage curve to the new object type. (If you close this window by accident, click the button "" Standardize")
    b) Click on an object type in the left list, then click on a damage curve from the right list, which describes your new object type best.
    c) Repeat this process for all new object types in the right list.
    d) Click "Okay" when you standardized all curves.
7. Click "Add to model" and your classification is updated with your data.

::: {.callout-warning}
##### Optional settings for spatially joining exposure points and finished floor height data
The current version of the Delft-FIAT model-builder only allows to update the primary OR the secondary object type. It is not possible to update one after another. The data will be overwritten. Nevertheless, this causes no problems when assigning the damage curve since this is done manually during the standardization step.
:::
