...
The user has the option to adjust the finished floor height default values of the default model (NSI or OSM) by importing a point- or polygon vector file with different data values for specific areas or assets. Moreover, the user may also import a finished floor height vector file for the whole model domain. 

![Workflow to refine the finished floor height.](../../_static/figures/finished_floor_height.svg){#fig-finished_floor_height}

::: {.callout-note}
The FloodAdapt Model Builder performs a spatial joint of the exposure assets with the point-/polygon finished floor height vector file and assigns the new values to each asset.
:::
...
To import ground elevation data, the user must provide a DEM raster file with the updated elevation values. The FloodAdapt Model Builder calculates the mean value from the pixels within the shape of each asset and assigns this value to the associated asset.   
...
When refining the max. potential damages it is necessary to distinguish between *structure* and *content* damage. The user can update one at a time with the following steps. 

::: {.callout-note}
The FloodAdapt model builder performs a spatial joint of the exposure assets with the point-/polygon max. potential damages vector file and assigns the new values to each asset.
:::

The user has the option to adjust the default values by importing a point- or polygon vector file with different data values for specific areas or to import a new max. potential Damages file for the complete model domain. 
...
<u>*Step-by-step instructions to setup/update the Max. Potential Damages*</u>   
  ...
  3. Load the file. This will load the data columns in your file into the **Attribute ID** drop-down menu.
  4. Select the column that holds the max. potential damage data (either "strucutral"or "content").
