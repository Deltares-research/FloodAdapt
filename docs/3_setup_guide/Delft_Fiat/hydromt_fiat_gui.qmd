...
3. Select the correct data source for your model. For a **U.S.** based model start with **National Structure Inventory** data. For a **global model** start with **Open Street Map** data.
  ...
  + <u>**Only global model (OSM)**</u>:  
    *Damage currency:* The default JRC damage currency is in **Euro**.
    ...
    ::: {.callout-warning}
    Don't change the default damage currency from Euro to another currency for the global model.There is no currency converter tool implemented in the FloodAdapt Model Builder yet.
    :::

    *Finished floor height:* Assign a value (meters) for the finished floor height to all assets at once.

    ::: {.callout-tip}
    The user can refine the finised floor height for the individual assets in the **Exposure**- >> **Finished Floor Height**-tab.
    :::

4. Include **OSM roads** by checking the checkbox "Include OSM roads".
5. Add to model.

The videos tutorials demonstrate the exposure setup using the NSI data for exposure and OSM data for roads (@fig-nsi_exposure) as well as the exposure setup for a global model, using OSM data (incl. roads; @fig-osm_exposure).
...
The following sections in this chapter elaborate on how the user can refine the model or import their own data. 

### Refine Finished Floor Height {#sec-finished_floor_height}
The user has the option to **adjust** the finished floor height **default values** of the base model (NSI or OSM) by importing a point- or polygon vector file (e.g. *.gpkg*, *.geojson*) with different data values for individual assets, specific areas or for the whole model domain. 

![Workflow to refine the finished floor height.](../../_static/figures/finished_floor_height.svg){#fig-finished_floor_height}

::: {.callout-note}
The FloodAdapt Model Builder performs a spatial joint of the exposure assets with the point-/polygon finished floor height vector file and assigns the new values to the intersecting/nearest asset.
:::

<u>*Step-by-step instructions to import the finished floor height*</u>  
...
5. *optional settings*: Define the method for the spatial joint. The default for point data is *'nearest'* and for polygon data *'intersection'*. The default will be automatically selected by the FloodAdapt Model Builder. You can keep the default settings if you don't wish to change the method.
6. Add to model.
...
To import ground elevation data, the user must provide a DEM raster file (e.g. *.tif*) with the updated elevation values. The FloodAdapt Model Builder calculates the mean value from the pixels within the shape of each asset and assigns this value to the associated asset.   

![Workflow to refine the ground elevation.](../../_static/figures/ground_elevation.svg){#fig-ground_elevation}

<u>*Step-by-step instructions to import ground elevation data*</u>   
...
2. There are two options in the drop-down menu to set up the ground elevation data:
    1. **SFINCS Ground Elevation**
        a. If you have already a SFINCS model you can use the same ground elevation from your SFINCS model. Select the  **subgrid-folder** of the SFINCS model, in which the dep_subgrid.tif-file is located. Do <u>not</u> select the file, simply the folder, the FloodAdapt Model Builder will recognize the file automatically.
        b. Add to model.
    2. **Upload file**
...
The user has the option to **adjust** the **default values** for the max. potential damages by importing a point- or polygon vector file (e.g. *.gpkg*, *.geojson*) with different data values for individual assets,specific areas or the complete model domain. 
When refining the max. potential damages it is necessary to distinguish between **structure** and **content** damage.

::: {.callout-note}
The FloodAdapt model builder performs a spatial joint of the exposure assets with the point-/polygon max. potential damages vector file and assigns the new values to the intersecting/nearest asset.
:::
...
<u>*Step-by-step instructions to import the Max. Potential Damages*</u>   
  ...
  5. Select the Damage Type either *structural* or *content*, depending on your data.  
  6.  *optional settings*: Define the method for the spatial joint. The default for point data is *'nearest'* and for polygon data *'intersection'*. The default will be automatically selected by the FloodAdapt Model Builder. You can keep the default settings if you don't wish to change the method.
  7. Add to model.
...
<u>*Step-by-step instructions to assign vulnerability curves*</u>

  1. Open the **"Vulnerability"**-tab on the top panel (@fig-vulnerability_curve). 
  2. The FloodAdapt Model Builder will **automatically** select the **default data source** based upon your exposure data (NSI or global). Therefore, you only need to add the curves to the model without further adjustment. 
...
However, the user has the freedom to upload any social vulnerability index and/or equity layer in form of a **vector file** (e.g. *.gpkg*, *.geojson*) and assign a value to each asset that overlaps the layer at any location (please refer to section [Additional Attributes](#sec-add_attributes)).

<u>*Step-by-step instructions to get SVI and equity data*</u>  
...
<u>*Step-by-step instructions to add additional attributes*</u>

If you wish to add any **aggregation** layer or **additional attribute** layer to your model follow these intstructions. The same instructions apply for SVI and/or Equity if you wish to add these to your model. 
