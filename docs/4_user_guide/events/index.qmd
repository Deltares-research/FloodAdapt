...
When a user selects a [Historical event without gauged water levels](historic_events_ungauged.qmd), the offshore model will first be run using the wind and pressure data from the [NOAA Global Forecast System](https://www.ncei.noaa.gov/products/weather-climate-models/global-forecast) (GFS). The calculated nearshore water levels will then be input to the overland model to calculate the overland flooding using the user-specified local wind, rainfall, and river discharge values.
