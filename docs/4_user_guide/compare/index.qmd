...
lightbox: auto
---
Being able to compare scenarios is important when addressing user questions. For example, consider the question "How would a historic storm look with sea level rise?" This question is best answered by simulating the historic storm without and then with sea level rise and comparing the output to evaluate where and by how much flooding and impacts would increase.
...
::: {.callout-note}
## Automatic scenario filtering
Similar to the spatial comparison tab, once a scenario has been added to the metric comparison box, the scenario selection box will update to only include scenarios of the same type - either a risk scenario or event scenario. Because the output differs between these scenario types, only scenarios of the same type can be compared.
:::