...
lightbox: auto
---
Flood adaptation measures are designed to reduce flood risk, and their effectiveness is typically assessed by calculating their risk-reduction **benefits** and the benefit-cost ratio, which compares benefits with the implementation and maintenance costs of the measure(s).
...
@fig-benefitTab shows the Benefits tab in FloodAdapt. The window is separate into a top panel with a map window on the left and, when using the collapsible frame, a time series of benefits on the right. The lower panel has a table on the left containing any benefit analyses the user has created. Next to it, there are buttons to add, edit, or delete a benefit analysis. When a benefit analysis is selected, the measures in the strategy for which benefits are being calculated will appear in the map windowd. The bottom panel will show a summary of the user-provided input to the benefit analysis, such as discount rate or end-year of the analysis. On the right side of the lower panel there is a "Calculate benefits" button. Once the user clicks this button, output will appear in the panel. This output consists of the (present value of the) benefits and a benefit-cost ratio (if cost information was provided). 

![**The FloodAdapt Benefits tab**](../_static/images/Benefits_tab.png){width=70% fig-align=left #fig-benefitTab}

The benefits can also be viewed spatially, and the lower panel contains map visualization options. The user can click "Aggregated benefits" to see the spatial distribution of benefits (see @fig-benefitDist for an example). They can choose from the "Aggregation" drop-down menu which aggregation level they would like to view, for example neighborhoods or census blocks. 

![**Visualizing the benefits spatially**](../_static/images/Benefits_distributedBenefits.png){width=70% fig-align=left #fig-benefitDist}

In the map visualization options, there is also an equity-weighted button. When this is checked the distributed (aggregated) benefits are displayed using equity weights, see @fig-benefitEquity. The figure shows the benefits without equity weights on the left and with equity weights on the right. Blue stars highlight lower-income census-block groups. When equity weights are applied, the benefits of these lower-income areas will be weighted more heavily; in contrast, high-income area benefits will be weighted less heavily. Equity weights are calculated in FloodAdapt using information on income. Risk estimates are multiplied by the equity weights to derive equity-weighted risk estimates, from which equity-weighted benefits are calculated. This visualization is only available at the same scale for which income data is available. This will typically be census block level. 

![**Visualizing equity-weighted benefits. The blue stars indicate lower-income census block groups. Their benefits are more weighted more heavily when equity-weights are applied (right side versus left side)**](../_static/images/benefits_equity.png){width=70% fig-align=left #fig-benefitEquity}

## Creating a new benefit analysis
FloodAdapt users can evaulate the risk-reduction benefits of any strategy they have created (which can be composed of a single measure or a collection of measures). To do this, go to the Benefits tab in FloodAdapt, and in the bottom panel click on "Add benefit analysis". A popup window will appear within which you will specify important choices about your benefit analysis. 
...
* **Situation at implementation**: The implementation situation is selected for you. It consists of the 'current' projection representing current climate and socio-economic conditions. The implementation year is defined during system setup.  
* **Situation at end of analysis**: In the Project end-of-life situation, specify the year you consider to be the end-of-lifetime of your project. This can also be the year corresponding to the duration over which you want to calculate benefits. In @fig-benefitPopup, the year 2060 is specified. That means benefits will be calculated between the current year and 2060. From the "Projection" dropdown menu, select the climate and/or socio-economic conditions you expect at the end of the lifetime of your project. For example, if you expect sea level rise to be 1 foot at the year specified (e.g. 2060 in @fig-benefitPopup), then choose a projection with 1 foot of SLR. Note that you must have already [created such a projection](projections/index.qmd) in order to select it here. The projection dropdown menu will contain all projections that have been created in the Projections tab. 
* **Costs**: If you have information on the implementation and maintenance costs of your project, you can set "Add costs" to "Yes" and a "Costs" panel will appear. Here you can enter the costs. If you do not have this information, set "Add costs" to "No" and the "Costs" panel will disappear. When cost information is not available, FloodAdapt will only calculate the risk-reduction benefits. When cost information is available, it will additionally calculate the benefit-cost ratio.
...
* **Create missing scenarios**: If any of the required scenarios is missing when you click "Check scenarios", you can click "Create missing scenarios" and FloodAdapt will automatically create the missing scenarios and place them in the scenarios table in the [Scenarios](scenarios/index.qmd) tab. Once this has been done (the names wil update in the "scenario created" column)
* **Click "OK" to add the analysis to the Benefits tab**

![**The "Check scenarios" feature in the Benefit analysis popup window**](../_static/images/benefits_scenarioCheck.png){width=70% fig-align=left #fig-benefitCheckScenarios}

:::{.callout-note}
## Required scenarios need to be run before running your benefit analysis
The "Check scenarios" and "Create missing scenarios" buttons guide users to ensure the required scenarios are created for the benefit analysis. However, scenarios can only be run from the [Scenarios](scenarios/index.qmd) tab. If any of the four required scenarios have not been run, you need to go to the Scenarios tab and run them. When some of the required scenarios have not yet been run, the "Calculate benefits" button will be greyed out and you will see text beneath it informing you that "You cannot calculate benefits before running th required scenarios (use Scenarios tab)", see @fig-benefitCalcFirst.
...
Once the required scenarios have been simulated, the "Calculate benefits" button will be active, see @fig-benefitReady. When the button is clicked, the benefit calculation happens on-the-fly, but because the risk information is already calculated, the results appear instantly. 
...
The benefits results consist of the present value of the benefits and (if cost information provided) the benefit-cost ratio (BCR). These are both printed in the lower panel of the Benefits tab. The collapsible frame contains a graphic (see @fig-benefitTS) with the risk-reduction calculated under current conditions and under future conditions (green dots in the figure), the interpolated benefits over time (black line), the discounted benefits over time, and the area under the dicsounted benefits curve representing the present value of the benefits. 

![**Time series output of the benefits**](../_static/images/benefits_TS.png){width=70% fig-align=left #fig-benefitTS}


### How does the benefit calculation work?
The benefit calculation works as follows:
...
4. The **annual discounted benefits** ($B_d(t)$) are calculated as 
    ...
    where $B(t)$ is the the risk-reduction benefit in year $t$, where $t$ is the year since implementation, and $r$ is the discount rate. 

5. The **(present value of the) benefits** are calculated as the sum of the annual discounted benefits over the analysis duration, represented in @fig-benefitTS by the green shaded area. 

All of these calculation steps are saved in the output folder