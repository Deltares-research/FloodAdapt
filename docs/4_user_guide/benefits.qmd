...
The benefits results consist of the present value of the benefits and (if cost information provided) the benefit-cost ratio (BCR). These are both printed in the lower panel of the Benefits tab. The collapsible frame contains a graphic (see @fig-benefitTS) with the risk-reduction benefits calculated under current conditions and under future conditions (green dots in the figure), the interpolated benefits over time (black line), the discounted benefits over time (dark green line), and the area under the dicsounted benefits curve (green shading) representing the present value of the benefits. 
...
1. The **risk-reduction benefit under current conditions** is calculated as the difference in the risk without the strategy and with the strategy under the "current" projection (representing current conditions). This is the green dot for the current year in @fig-benefitTS.

2. The **risk-reduction benefit under future conditions** (for the year specified in the end-of-analysis situation) is calculated as the difference in the risk without the strategy and with the strategy under the future projection specified in the end-of-analysis situation when you created your benefit analysis. This is the green dot for the future year in @fig-benefitTS. 
...
5. The **(present value of the) benefits** are calculated as the sum of the annual discounted benefits over the analysis duration ($N$ years), represented in @fig-benefitTS by the green shaded area. 

    $Benefits\ =\ \sum_{t=0}^N B_d\ (t)\ $

In addition to the above results which can be viewed in the Output tab, FloodAdapt also calculates - when implementation and/or maintenance cost information is provided - the net present value and the internal rate of return. These are provided in a results text file (TOML file format) in the benefits output folder. 
...
All of these calculation steps are saved in the [output folder](#benefits-output-folder). 
