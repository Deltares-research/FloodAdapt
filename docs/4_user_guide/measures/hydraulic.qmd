...
![**Selection of Hydraulic measures in the Measures tab, shown here for the example of a floodwall**](../../_static/images/measures_buildingLevel_GUI.png){width=70% fig-align=left #fig-measures_buildingLevel_GUI}

To specify a hydraulic measure, the user goes to the Measures tab, and in the "Category" drop-down menu selects "Hydraulic" (see @fig-measures_buildingLevel_GUI). The user can then choose for "Measure" one of the three measure types: [floodwall or levee](#floodwalls-and-levees), or [pumps](#pumps). For each of these measures, the user will choose an "Area type". This indicates how the user will select the buildings to which they want to apply the measure. There are two options:
