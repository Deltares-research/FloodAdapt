...
class Elevate(ImpactMeasure):
    ...
    def set_default(self):
        ...
        self.selection_type = (
            None  # selection type (can be "aggregation area", "all" or "polygon")
        )
        self.aggregation_area = None  # name of area to use (this is needed if selection type is "aggregation_area")
        self.polygon_file = (
            None  # polygon file to use (this is needed if selection type is "polygon")
        )
        self.property_type = (
            "RES"  # this is the object type to be used to apply the measure on
        )
