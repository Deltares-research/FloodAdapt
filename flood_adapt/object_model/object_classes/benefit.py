...
from flood_adapt.object_model.models.benefits import BenefitModel
...
class Benefit(FAObject, IBenefit):
    ...
    def init(self):
        ...
        self.results_path = Path(self.database_input_path).parent.joinpath(
            "output", "Benefits", self._attrs.name
        )
    ...
    def check_scenarios(self):
        ...
        for scenario in scenarios_calc.keys():
            scenarios_calc[scenario]["event"] = self._attrs.event_set

            if "current" in scenario:
                scenarios_calc[scenario][
                    "projection"
                ] = self._attrs.current_situation.projection
            elif "future" in scenario:
                scenarios_calc[scenario]["projection"] = self._attrs.projection

            if "no_measures" in scenario:
                scenarios_calc[scenario]["strategy"] = self._attrs.baseline_strategy
            else:
                scenarios_calc[scenario]["strategy"] = self._attrs.strategy
    ...
    def cba(self):
        ...
        year_start = self._attrs.current_situation.year
        year_end = self._attrs.future_year
        ...
        cba = Benefit._calc_benefits(
            ...
            discount_rate=self._attrs.discount_rate,
        )
        ...
        cost_calc = (self._attrs.implementation_cost is not None) and (
            self._attrs.annual_maint_cost is not None
        )
        if cost_calc:
            cba = Benefit._calc_costs(
                benefits=cba,
                implementation_cost=self._attrs.implementation_cost,
                annual_maint_cost=self._attrs.annual_maint_cost,
                discount_rate=self._attrs.discount_rate,
            )
            # Calculate costs
    ...
    @classmethod
    def load_dict(cls, data: dict[str, Any], database_input_path: Union[str, os.PathLike]):
        """create Benefit object from dictionary, e.g. when initialized from GUI"""

        obj = super().load_dict(cls, data, database_input_path)
        obj.init()
        return obj
